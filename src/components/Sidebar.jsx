...
export function Sidebar({ collapsed, setCollapsed, mobileOpen = false, onClose }) {
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-50 h-screen w-52 bg-fbs-darker text-white flex flex-col justify-between transition-transform duration-300 overflow-hidden flex-shrink-0 ${
        mobileOpen ? "translate-x-0" : "-translate-x-full"
      } lg:static lg:translate-x-0 lg:z-auto lg:transition-all ${
        collapsed ? "lg:w-16" : "lg:w-52"
      }`}>
      <div>
        <div className={`${showLabels ? "px-5" : "px-2"} pt-6 pb-4`}>
          {/* Logo */}
          ...
          <button
            ...
            className="hidden lg:flex w-full h-9 items-center justify-center rounded-lg bg-fbs-card hover:bg-fbs-border transition mb-5">
                ...
                {sec.items.map((item) => {
                  ...
                  return (
                    <NavLink
                      ...
                      onClick={onClose}
                      className={({ isActive }) =>
                        `flex items-center ${showLabels ? "gap-3 px-3" : "justify-center px-0"} py-2 rounded-lg text-sm transition-all duration-200 ${
                          isActive
      ...
      {/* User + Signout */}
      <div className={`${showLabels ? "px-5" : "px-2"} pb-6`}>
        <div className={`flex items-center ${showLabels ? "gap-3" : "justify-center"} mb-3`}>
        ...
        <button
          ...
          className={`w-full text-sm border border-fbs-border rounded-lg py-2 hover:bg-fbs-card transition flex items-center gap-2 ${showLabels ? "px-3" : "justify-center px-0"}`}>
